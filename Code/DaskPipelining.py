...
def JobClassification(str1,str2):
    ...
    with open(str2,'r') as data: #Reading the lemmatised data
        ...
        column = [row[3]for row in reader2] #from job description column
        ...
        with open(str2, 'r') as data: # reading the lematized list
            reader2 = csv.reader(data)
            column_bank = [row[2] for row in reader2] # bank name
            Z = np.array(column_bank)
            print (Z[0])
        with open(str2, 'r') as data:
            reader2 = csv.reader(data)
            column_jobTitle = [row[4] for row in reader2] #job title
            A = np.array(column_jobTitle)
            print (A[0])
        path = "result.csv"
    with open(path,'w') as f:
        csv_write = csv.writer(f)
        csv_head = ["Job_ID","Bank_Name","Job_Title","Cluster_Name","The_amount_of_word_count","Category"]
...
dsk={'load1' :loading("ClustersList.csv"),
     'load2': loading("LemmatisedDataS3.csv"),
